...
export function ArchCell({ cell, position, lookup }: ArchCellProps) {
  ...
  return (
    <group name="archCell" position={position} rotation={archAxis === 'z' ? [0, Math.PI / 2, 0] : [0, 0, 0]}>
      <mesh name="archLeft" position={archAxis === 'x' ? [-0.32, 0, 0] : [0, 0, -0.32]} castShadow receiveShadow>
      ...
      <mesh name="archRight" position={archAxis === 'x' ? [0.32, 0, 0] : [0, 0, 0.32]} castShadow receiveShadow>
        <RoundedBox args={[archAxis === 'x' ? 0.28 : 0.3, 1.0, archAxis === 'x' ? 0.3 : 0.28]} radius={0.04} smoothness={4}>
          <meshStandardMaterial color={cell.color ?? '#9f8f7b'} roughness={0.88} />
        </RoundedBox>
      </mesh>
      <mesh name="archTop" position={[0, 0.3, 0]} castShadow receiveShadow>
        <torusGeometry args={[0.36, 0.1, 12, 32, Math.PI]} />
        <meshStandardMaterial color={cell.color ?? '#9f8f7b'} roughness={0.82} />
      </mesh>
      <mesh name="archBase" position={[0, 0.46, 0]} castShadow receiveShadow>
